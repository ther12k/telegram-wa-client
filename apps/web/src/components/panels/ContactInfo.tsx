...
import { X, Search, Bell, BellOff, Image as ImageIcon, FileText, Link2, Star, Trash2, LogOut, ChevronRight } from 'lucide-react'
    ...
    <div className="h-full w-full bg-white dark:bg-[#111b21] flex flex-col animate-slide-in-right">
      ...
      <div className="flex-1 overflow-y-auto slim-scroll">
        <div className="pt-8 pb-6 flex flex-col items-center border-b border-slate-100 dark:border-white/5">
          ...
          <div className="mt-4 text-xl font-semibold text-slate-900 dark:text-white">{chat.name}</div>
        ...
        {chat.kind === 'private' && (
          <div className="px-5 py-4 border-b border-slate-100 dark:border-white/5">
            <div className="text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500 mb-1">About</div>
        ...
        <div className="py-2">
          {[
            { icon: muted ? BellOff : Bell, label: muted ? 'Muted' : 'Notifications', value: '', onClick: () => setMuted(!muted) },
          ...
          ].map((it: { icon: React.ComponentType; label: string; value?: string; onClick?: () => void }) => (
            <button
              key={it.label}
              onClick={it.onClick}
            ...
            >
              ...
              {it.value && <span className="text-xs text-slate-400 dark:text-slate-500">{it.value}</span>}
        ...
        {(chat.kind === 'group' || chat.kind === 'channel') && (
          <div className="border-t border-slate-100 dark:border-white/5 py-2">
            <div className="px-5 py-2 text-xs uppercase tracking-wide text-slate-400 dark:text-slate-500">Members</div>
            {Object.values(people).slice(0, 5).map((p) => (
              <div key={p.name} className="flex items-center gap-3 px-5 py-2.5 hover:bg-slate-50 dark:hover:bg-[#1f2c33]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{p.about}</div>
                </div>
              </div>
            ))}
          </div>
